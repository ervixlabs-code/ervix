...
export default function ContactPageContentEditPage() {
  const params = useParams()
  const router = useRouter()
  const searchParams = useSearchParams()

  const routeId = params?.id
  const slugFallback = searchParams.get("slug")
  ...
  useEffect(() => {
    ...
    async function loadItem() {
      try {
        ...
        if (routeId) {
          url = `${ENDPOINT}/${routeId}`
        } else if (slugFallback) {
          url = `${ENDPOINT}/slug/${slugFallback}`
        } else {
          throw new Error("Düzenlenecek kayıt id bilgisi bulunamadı.")
        }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const id = form.id || routeId
      if (!id) {
        throw new Error("Güncellenecek kayıt id bilgisi bulunamadı.")
      }
      ...
      if (!res.ok) {
        const text = await res.text()
        throw new Error(text || "Kayıt güncellenemedi.")
      }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      console.error("Update contact page content error:", err)
      setError(err?.message || "Kayıt güncellenirken hata oluştu.")
    } finally {
      setSubmitting(false)
    }
  }
      ...
      <div className="min-h-screen bg-[#f6f8fc] text-slate-900">
        ...
        <div className="pl-0 lg:pl-[300px]">
          <main className="mx-auto max-w-[1780px] px-4 py-6 sm:px-6">
            <div className="flex min-h-[320px] items-center justify-center rounded-[28px] border border-slate-200 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
              <div className="flex items-center gap-3 text-slate-500">
                <Loader2 className="h-5 w-5 animate-spin" />
                <span className="text-sm font-medium">Kayıt getiriliyor...</span>
              </div>
            </div>
          </main>
        </div>
              ...
              <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                    Sol tarafta input alanları, sağ tarafta canlı preview yer alır.
                    Ekstra topbar yoktur ve kaydetme doğrudan update endpointine gider.
                  </p>
                </div>
                  ...
                  <div className="sticky bottom-4 z-20 rounded-[24px] border border-slate-200 bg-white/95 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.10)] backdrop-blur">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div className="text-sm text-slate-500">
                        Bu buton mevcut kaydı PATCH ile günceller.
                      </div>
