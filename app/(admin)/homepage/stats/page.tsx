...
function statusClasses(status: StatStatus) {
  return status === "Aktif"
...
function mapApiItem(item: ApiStatItem): StatItem {
  return {
    id: Number(item.id),
    value: item.value || "",
    title: item.title || "",
...
export default function HomePageStatsListPage() {
  ...
  async function fetchStats() {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          Array.isArray(result?.message)
            ? result.message.join(", ")
            : result?.message || "İstatistik verileri alınamadı."
        )
      }
      ...
      alert(error?.message || "İstatistik verileri alınamadı.")
      setStatItems([])
    } finally {
      setLoading(false)
    }
  ...
  async function handleDelete(id: number) {
    const confirmed = window.confirm("Bu istatistik kaydını silmek istediğine emin misin?")
    if (!confirmed) return
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          Array.isArray(result?.message)
            ? result.message.join(", ")
            : result?.message || "İstatistik kaydı silinemedi."
        )
      }

      setStatItems((prev) => prev.filter((item) => item.id !== id))
    } catch (error: any) {
      console.error("Stats delete error:", error)
      alert(error?.message || "İstatistik kaydı silinemedi.")
    } finally {
      setDeletingId(null)
    }
  }
  ...
  const activeStats = statItems.filter((item) => item.status === "Aktif").length
  const passiveStats = statItems.filter((item) => item.status === "Pasif").length
  const highestOrder =
    statItems.length > 0 ? Math.max(...statItems.map((item) => item.sortOrder)) : 0
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Anasayfadaki istatistik kartlarının değer, başlık, açıklama,
              durum ve sıralama bilgilerini buradan yönetebilirsin.
            </p>
          </div>

          <div className="flex flex-wrap gap-3">
            <Link
              href="/homepage/stats/create"
        ...
        {[
          {
            ...
            note: "Sistemde kayıtlı istatistik",
            icon: BarChart2,
          },
          {
            title: "Aktif Kart",
            value: activeStats.toString(),
            note: "Yayında olan kartlar",
            icon: CheckCircle2,
          },
          {
            title: "Pasif Kart",
            value: passiveStats.toString(),
        ...
        <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Arama yap, filtrele ve istatistik kartlarını yönet
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row">
            <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
              ...
              <input
                ...
                placeholder="Value, başlık veya açıklama ara..."
        ...
        {loading && (
          <div className="rounded-[22px] border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              İstatistik kayıtları getiriliyor, lütfen bekleyin.
            </p>
          </div>
        )}
          ...
          <>
              ...
              {paginatedItems.map((item) => (
                ...
                >
                  ...
                  <div className="mt-4 rounded-xl bg-white px-3 py-2">
                    ...
                    <p className="mt-1 font-medium text-slate-700">{item.createdAt}</p>
                  </div>

                  <div className="mt-4 flex flex-wrap gap-2">
                    <Link
                      href={`/homepage/stats/${item.id}`}
                    ...
                    <Link
                      href={`/homepage/stats/${item.id}/edit`}
                      className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"
                    >
            ...
            <div className="hidden overflow-hidden rounded-[22px] border border-slate-200 xl:block">
              <div className="grid grid-cols-12 bg-slate-50 px-5 py-4 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                <div className="col-span-4">İstatistik</div>
            ...
            {paginatedItems.length === 0 && (
              <div className="rounded-[22px] border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center">
                <p className="text-lg font-semibold text-slate-700">İstatistik bulunamadı</p>
                <p className="mt-2 text-sm text-slate-500">
