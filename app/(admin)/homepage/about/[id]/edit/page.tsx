...
type ApiAboutItem = {
  ...
  status: "Aktif" | "Pasif" | "AKTIF" | "PASIF"
  sortOrder: number
}
...
function mapApiStatusToUi(status?: string): AboutStatus {
  return status === "Pasif" || status === "PASIF" ? "Pasif" : "Aktif"
}

function mapUiStatusToApi(status: AboutStatus) {
  return status === "Aktif" ? "AKTIF" : "PASIF"
}

function mapApiItemToForm(item: ApiAboutItem): AboutItem {
  return {
    id: Number(item.id),
    badge: item.badge || "",
    title: item.title || "",
...
export default function EditAboutPage() {
  ...
  const [infoCards, setInfoCards] = useState<{ title: string; text: string }[]>([])
  ...
  useEffect(() => {
    async function fetchItem() {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE}/homepage/about/${id}`, {
          method: "GET",
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            Array.isArray(result?.message)
              ? result.message.join(", ")
              : result?.message || "About kaydı getirilemedi."
          )
        }

        const item = mapApiItemToForm(result as ApiAboutItem)
        ...
        setTags(item.tags)
        setInfoCards(item.infoCards)
      } catch (error: any) {
        console.error("About detail fetch error:", error)
        alert(error?.message || "About kaydı getirilemedi.")
      } finally {
        setIsLoaded(true)
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          Array.isArray(result?.message)
            ? result.message.join(", ")
            : result?.message || "About kaydı güncellenemedi."
        )
      }

      alert("About kaydı başarıyla güncellendi.")
  ...
  if (notFound) {
    return (
      <div className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <h1 className="text-2xl font-bold text-slate-900">About kaydı bulunamadı</h1>
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Seçtiğin about kaydının ana içerik, tag, featured card ve alt kart alanlarını güncelleyebilirsin.
            </p>
          </div>
        ...
        <div className="2xl:col-span-7 space-y-6">
            ...
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Badge, başlık ve açıklamaları güncelle
                </p>
              </div>
            </div>
          ...
          <section className="flex flex-col gap-3 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Form bilgilerini kontrol edip about kaydını güncelleyebilirsin.
              </p>
            </div>
