...
function mapUiStatusToApi(status: ShowcaseStatus) {
  return status === "Aktif" ? "AKTIF" : "PASIF"
}

export default function EditShowcase3DPage() {
  ...
  const [spots, setSpots] = useState<{ title: string; description: string }[]>([])
  ...
  useEffect(() => {
    async function fetchItem() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            Array.isArray(result?.message)
              ? result.message.join(", ")
              : result?.message || "3D showcase kaydı getirilemedi."
          )
        }

        const item = result as ApiShowcaseItem
        ...
        console.error("Showcase 3D detail fetch error:", error)
        alert(error?.message || "3D showcase kaydı getirilemedi.")
      } finally {
        setIsLoaded(true)
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          Array.isArray(result?.message)
            ? result.message.join(", ")
            : result?.message || "3D showcase kaydı güncellenemedi."
        )
      }

      alert("3D showcase kaydı başarıyla güncellendi.")
      router.push("/homepage/showcase-3d")
    } catch (error: any) {
      console.error("Showcase 3D update error:", error)
  ...
  if (notFound) {
    return (
      <div className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <h1 className="text-2xl font-bold text-slate-900">Kayıt bulunamadı</h1>
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Seçtiğin showcase kaydının metin alanlarını, embed HTML kodunu ve info spotlarını güncelleyebilirsin.
            </p>
          </div>
        ...
        <div className="2xl:col-span-7 space-y-6">
            ...
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Başlık, açıklama ve CTA alanlarını güncelle
                </p>
              </div>
            </div>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:p-6">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Web tarafında preview olarak gösterilecek HTML kodunu güncelle
                </p>
              </div>
            </div>
          ...
          <section className="flex flex-col gap-3 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Form bilgilerini kontrol edip showcase kaydını güncelleyebilirsin.
              </p>
            </div>
