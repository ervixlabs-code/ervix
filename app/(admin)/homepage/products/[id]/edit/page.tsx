...
function mapUiStatusToApi(status: ProductStatus) {
  ...
  useEffect(() => {
    async function fetchItem() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            Array.isArray(result?.message)
              ? result.message.join(", ")
              : result?.message || "Ürün kaydı getirilemedi."
          )
        }

        const item = result as ApiProductItem
        ...
        console.error("Products detail fetch error:", error)
        alert(error?.message || "Ürün kaydı getirilemedi.")
      } finally {
        setIsLoaded(true)
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          Array.isArray(result?.message)
            ? result.message.join(", ")
            : result?.message || "Ürün kaydı güncellenemedi."
        )
      }

      alert("Ürün kaydı başarıyla güncellendi.")
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Seçtiğin ürün kartının içerik, tag ve preview alanlarını güncelleyebilirsin.
            </p>
          </div>
            ...
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Başlık, badge ve açıklamayı güncelle
                </p>
              </div>
            </div>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:p-6">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Kart içindeki alt preview alanlarını güncelle
                </p>
              </div>
            </div>
