...
type ApiProductItem = {
  ...
  detailButtonText: string
  detailButtonUrl: string
  status: ProductStatus | "AKTIF" | "PASIF"
  sortOrder: number
  createdAt: string
}
...
function mapApiStatusToUi(status?: string): ProductStatus {
...
function mapApiItem(item: ApiProductItem): ProductItem {
  return {
    id: Number(item.id),
    title: item.title || "",
    badge: item.badge || "",
    ...
    detailButtonText: item.detailButtonText || "",
    detailButtonUrl: item.detailButtonUrl || "",
...
export default function HomePageProductsListPage() {
  ...
  async function fetchProducts() {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          Array.isArray(result?.message)
            ? result.message.join(", ")
            : result?.message || "Ürün verileri alınamadı."
        )
      }
      ...
      alert(error?.message || "Ürün verileri alınamadı.")
      setProductItems([])
    } finally {
      setLoading(false)
    }
  ...
  async function handleDelete(id: number) {
    const confirmed = window.confirm("Bu ürün kaydını silmek istediğine emin misin?")
    if (!confirmed) return

    try {
      setDeletingId(id)

      const response = await fetch(`${API_BASE}/homepage/products/${id}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
        },
      ...
      if (!response.ok) {
        throw new Error(
          Array.isArray(result?.message)
            ? result.message.join(", ")
            : result?.message || "Ürün kaydı silinemedi."
        )
      }
      ...
      alert(error?.message || "Ürün kaydı silinemedi.")
    } finally {
      setDeletingId(null)
    }
  }
  ...
  const filteredItems = useMemo(() => {
    return productItems.filter((item) => {
      ...
      const matchesSearch =
        ...
        item.previewSubtitle.toLowerCase().includes(q) ||
        item.categoryText.toLowerCase().includes(q) ||
        item.focusText.toLowerCase().includes(q) ||
        item.detailButtonText.toLowerCase().includes(q) ||
        item.detailButtonUrl.toLowerCase().includes(q) ||
  ...
  }, [productItems, search, statusFilter])
  ...
  const activeItems = productItems.filter((item) => item.status === "Aktif").length
  const passiveItems = productItems.filter((item) => item.status === "Pasif").length
  const highestOrder =
    productItems.length > 0 ? Math.max(...productItems.map((item) => item.sortOrder)) : 0
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-3xl xl:text-4xl">
              Products Alanını Yönet
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Products bölümündeki ürün kartlarını, tagleri, preview içeriklerini ve buton alanlarını buradan yönetebilirsin.
            </p>
          </div>

          <div className="flex flex-wrap gap-3">
            <Link
              href="/homepage/products/create"
        ...
        {[
          {
            ...
            note: "Products alanı kayıtları",
            icon: Package2,
          },
          {
            title: "Aktif Ürün",
            value: activeItems.toString(),
            note: "Yayında olan ürünler",
            icon: CheckCircle2,
          },
          {
            title: "Pasif Ürün",
        ...
        <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Arama yap, filtrele ve ürün kartlarını yönet
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row">
            <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
              ...
              <input
                ...
                placeholder="Ürün adı, badge veya açıklama ara..."
        ...
        {loading && (
          <div className="rounded-[22px] border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Ürün kayıtları getiriliyor, lütfen bekleyin.
            </p>
          </div>
        )}
          ...
          <>
              ...
              {paginatedItems.map((item) => (
                ...
                >
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="rounded-xl bg-white px-3 py-2">
                      <p className="text-xs text-slate-400">Detay Butonu</p>
                      <p className="mt-1 font-medium text-slate-700">{item.detailButtonText}</p>
                    </div>
                  </div>

                  <div className="mt-4 flex flex-wrap gap-2">
                    <Link
                      href={`/homepage/products/${item.id}`}
                    ...
                    <Link
                      href={`/homepage/products/${item.id}/edit`}
                      className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"
                    >
            ...
            <div className="hidden overflow-hidden rounded-[22px] border border-slate-200 xl:block">
              <div className="grid grid-cols-12 bg-slate-50 px-5 py-4 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                ...
                <div className="col-span-2 text-center">Tag / Badge</div>
                <div className="col-span-2 text-center">Detay Link</div>
