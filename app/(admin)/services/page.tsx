...
export default function ServicesListPage() {
  ...
  async function fetchItems() {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Servis kayıtları alınamadı.")
      }
      ...
      const mapped: ServiceItem[] = rawList.map((item: any) => ({
        id: String(item.id),
        title: item.title ?? "",
        slug: item.slug ?? "",
      ...
      setItems([])
      setError("Servis kayıtları yüklenirken hata oluştu.")
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete(id: string) {
    const ok = window.confirm("Bu servis kaydını silmek istediğine emin misin?")
    if (!ok) return
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const uiStatus = getStatusLabel(item.status)
  ...
  const totalFeatures = items.reduce((sum, item) => sum + item.features.length, 0)
              ...
              <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
                    Servis kayıtlarını, açıklamalarını, icon yapılarını, feature
                    alanlarını ve kullanılan teknolojileri buradan yönetebilirsin.
                  </p>
                </div>
              ...
              {[
                {
                  ...
                  note: "Sistemdeki tüm servis kayıtları",
                  icon: Layers3,
                },
                {
                  title: "Aktif Servis",
                ...
                },
                {
                  ...
                  note: "Kapalı durumdaki servisler",
                  icon: PauseCircle,
                },
                {
                  title: "Toplam Feature",
                  value: totalFeatures.toString(),
                  note: "Servislere bağlı feature sayısı",
                  icon: Wrench,
                },
              ].map((item) => {
              ...
              <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Servisleri ara, filtrele ve yönet
                  </p>
                </div>
                ...
                <>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-start gap-3">
                            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 text-white shadow-[0_10px_25px_rgba(99,102,241,0.18)]">
                              <Icon className="h-5 w-5" />
                            </div>
                          ...
                          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div className="rounded-xl bg-white px-3 py-3">
                              <p className="text-xs text-slate-400">Sort Order</p>
                              <p className="mt-1 font-semibold text-slate-700">
                                {item.sortOrder}
                              </p>
                            </div>
                  ...
                  <div className="hidden overflow-hidden rounded-[22px] border border-slate-200 xl:block">
                    <div className="grid grid-cols-12 bg-slate-50 px-5 py-4 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                      <div className="col-span-3">Servis</div>
                      <div className="col-span-2 text-center">Durum</div>
                      <div className="col-span-3 text-center">İçerik Sayıları</div>
                        ...
                        return (
                          ...
                          >
                            <div className="col-span-3 min-w-0">
                              ...
                              <div className="flex items-start gap-3">
                                <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 text-white shadow-[0_10px_25px_rgba(99,102,241,0.18)]">
                            ...
                            <div className="col-span-3 flex justify-center">
                              <div className="flex flex-wrap items-center justify-center gap-2">
                                <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-700">
                                  ...
                                  {item.features.length} Feature
                                </span>

                                <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-700">
                                  <Code2 className="h-3.5 w-3.5" />
                                  {item.technologies.length} Teknoloji
                                </span>
