...
function mapApiItemToUi(item: any): ProductItem {
  return {
    id: String(item.id),
    title: item.title || "",
    icon: item.icon === "Layers3" ? "Layers3" : "MonitorSmartphone",
...
export default function ProductsListPage() {
  ...
  async function fetchProducts() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Ürün kayıtları alınamadı.")
      }
  ...
  async function handleDelete(id: string) {
    const ok = window.confirm("Bu ürün kaydını silmek istediğine emin misin?")
    if (!ok) return
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text()
        throw new Error(text || "Ürün kaydı silinemedi.")
      }

      setItems((prev) => prev.filter((item) => item.id !== id))
    } catch (error) {
      console.error("Delete product error:", error)
      alert("Ürün kaydı silinirken hata oluştu.")
    }
  }
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Products sayfasında yer alan ürün kartlarını, açıklamaları ve
              içerik alanlarını buradan yönetebilirsin.
            </p>
          </div>

          <div className="flex flex-wrap gap-3">
            <Link
              href="/products/list/create"
        ...
        <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Arama yap, filtrele ve ürün kayıtlarını yönet
            </p>
          </div>
          ...
          <>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <Link
                        href={`/products/list/${item.id}/edit`}
                        className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"
                      >
            ...
            <div className="hidden overflow-hidden rounded-[22px] border border-slate-200 xl:block">
              <div className="grid grid-cols-12 bg-slate-50 px-5 py-4 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                <div className="col-span-5">Ürün İçeriği</div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="col-span-2 flex justify-center">
                        <div className="inline-flex items-center rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700">
                          {item.category}
                        </div>
                      </div>
