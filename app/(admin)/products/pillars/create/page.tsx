...
export default function CreateProductPillarPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Product pillar kaydı oluşturulamadı.")
      }

      router.push("/products/pillars")
    } catch (error) {
      console.error("Create product pillar error:", error)
      alert("Product pillar kaydı oluşturulurken hata oluştu.")
    } finally {
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-3xl xl:text-4xl">
              Yeni Product Pillar Oluştur
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Products sayfasındaki yaklaşım kartları için yeni pillar kaydı
              oluşturabilirsin.
            </p>
          </div>
          ...
          <section className="flex flex-col gap-3 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                ...
                {isSubmitting ? "Kaydediliyor..." : "Pillar Kaydet"}
              </button>
            </div>
          </section>
        </div>
