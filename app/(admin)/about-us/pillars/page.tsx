...
type PillarStatusApi = "ACTIVE" | "PASSIVE" | "DRAFT"
type PillarStatusUi = "Aktif" | "Pasif" | "Taslak"
type IconKey = "Lightbulb" | "ShieldCheck" | "Rocket"
...
function statusClasses(status: PillarStatusUi) {
  if (status === "Aktif") {
...
function toUiStatus(status: PillarStatusApi): PillarStatusUi {
  switch (status) {
    case "ACTIVE":
      return "Aktif"
    case "PASSIVE":
...
const PILLARS_ENDPOINT = `${API_BASE}/about-us/pillars`

export default function AboutUsPillarsListPage() {
  ...
  async function fetchItems() {
    try {
      ...
      const res = await fetch(PILLARS_ENDPOINT, {
        method: "GET",
        cache: "no-store",
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Pillar kayıtları alınamadı.")
      }
      ...
      const mapped: PillarItem[] = rawList.map((item: any, index: number) => ({
        id: Number(item.id ?? index + 1),
        title: item.title ?? "",
        description: item.description ?? "",
        icon: (item.icon ?? "Lightbulb") as IconKey,
        status: toUiStatus(item.status as PillarStatusApi),
      ...
      console.error("Pillars list fetch error:", error)
      setError("Pillar kayıtları yüklenirken hata oluştu.")
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  async function handleDelete(id: number) {
    const ok = window.confirm("Bu pillar kaydını silmek istediğine emin misin?")
    if (!ok) return

    try {
      setDeletingId(id)

      const res = await fetch(`${PILLARS_ENDPOINT}/${id}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
        },
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-3xl xl:text-4xl">
              Pillars Alanını Yönet
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Why ERVIX bölümündeki kartları, ikonları ve sıralamayı buradan yönetebilirsin.
            </p>
          </div>
        ...
        <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Arama yap, filtrele ve pillar kartlarını yönet
            </p>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <Link
                        href={`/about-us/pillars/${item.id}/edit`}
                        className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"
                      >
