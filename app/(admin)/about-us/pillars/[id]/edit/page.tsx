...
export default function EditAboutPillarPage() {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        ...
        console.error("Edit about pillar fetch error:", error)
        setNotFound(true)
      } finally {
        setIsLoaded(true)
      }
  ...
  const preview = useMemo(() => {
    return {
      title: form.title || "Product Thinking",
      description:
        form.description ||
        "Sadece teslim etmiyor, ürün mantığıyla uzun ömürlü ve sürdürülebilir dijital yapılar kuruyoruz.",
      icon: form.icon,
      status: form.status,
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.title.trim()) {
      alert("Lütfen pillar başlığını gir.")
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text()
        throw new Error(text || "Pillar kaydı güncellenemedi.")
      }

      router.push("/about-us/pillars")
    } catch (error) {
      console.error("Edit about pillar submit error:", error)
      alert("Pillar kaydı güncellenirken hata oluştu.")
    } finally {
  ...
  if (notFound) {
    return (
      <div className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <h1 className="text-2xl font-bold text-slate-900">Pillar kaydı bulunamadı</h1>
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-3xl xl:text-4xl">
              Pillar Kaydını Düzenle
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Why ERVIX bölümündeki kartı, ikonunu ve sıralamasını güncelleyebilirsin.
            </p>
          </div>
