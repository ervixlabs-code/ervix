...
type HeroStatusApi = "ACTIVE" | "PASSIVE" | "DRAFT"
type HeroStatusUi = "Aktif" | "Pasif" | "Taslak"
...
function statusClasses(status: HeroStatusUi) {
  if (status === "Aktif") {
...
function toUiStatus(status: HeroStatusApi): HeroStatusUi {
  switch (status) {
    case "ACTIVE":
      return "Aktif"
    case "PASSIVE":
...
export default function AboutUsHeroListPage() {
  ...
  async function fetchItems() {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Hero kayıtları alınamadı.")
      }
      ...
      const mapped: HeroItem[] = rawList.map((item: any, index: number) => ({
        id: item.id ?? index + 1,
        eyebrow: item.eyebrow ?? "",
        title: item.title ?? "",
        description: item.description ?? "",
        primaryCtaLabel: item.primaryCtaLabel ?? item.primaryCta?.label ?? "",
        primaryCtaHref: item.primaryCtaHref ?? item.primaryCta?.href ?? "",
        secondaryCtaLabel: item.secondaryCtaLabel ?? item.secondaryCta?.label ?? "",
        secondaryCtaHref: item.secondaryCtaHref ?? item.secondaryCta?.href ?? "",
        tags: Array.isArray(item.tags) ? item.tags : [],
        panelTitle: item.panelTitle ?? "",
        panelDescription: item.panelDescription ?? "",
        status: toUiStatus(item.status as HeroStatusApi),
      ...
      console.error("Hero list fetch error:", error)
      setError("Hero kayıtları yüklenirken hata oluştu.")
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  async function handleDelete(id: number | string) {
    const ok = window.confirm("Bu hero kaydını silmek istediğine emin misin?")
    if (!ok) return
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesSearch =
        ...
        item.primaryCtaLabel.toLowerCase().includes(q) ||
        item.secondaryCtaLabel.toLowerCase().includes(q) ||
              ...
              <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
                    About Us sayfasındaki hero kayıtlarını, CTA alanlarını,
                    panel başlıklarını ve sıralamayı buradan yönetebilirsin.
                  </p>
                </div>
              ...
              <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Arama yap, filtrele ve hero alanlarını yönet
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-3 md:flex-row">
                  <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                    ...
                    <input
                      ...
                      placeholder="Eyebrow, başlık, açıklama veya panel başlığı ara..."
                ...
                <>
                      ...
                      >
                        ...
                        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                          ...
                          <p className="mt-2 font-semibold text-slate-800">
                            {item.panelTitle || "Panel Başlığı Yok"}
                          </p>
                          <p className="mt-1 text-sm leading-6 text-slate-500">
                            {item.panelDescription || "Panel açıklaması yok"}
                          </p>
                        </div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          ...
                          <Link
                            href={`/about-us/hero/${item.id}/edit`}
                            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"
                          >
                  ...
                  <div className="hidden overflow-hidden rounded-[22px] border border-slate-200 xl:block">
                    <div className="grid grid-cols-12 bg-slate-50 px-5 py-4 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                      <div className="col-span-4">Hero İçeriği</div>
                      <div className="col-span-2 text-center">Durum</div>
                      <div className="col-span-1 text-center">Sıra</div>
                      <div className="col-span-2 text-center">Panel</div>
                      <div className="col-span-2 text-center">CTA</div>
