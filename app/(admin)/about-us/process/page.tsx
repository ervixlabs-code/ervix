...
type ProcessStatusApi = "ACTIVE" | "PASSIVE" | "DRAFT"
type ProcessStatusUi = "Aktif" | "Pasif" | "Taslak"
...
function statusClasses(status: ProcessStatusUi) {
  if (status === "Aktif") {
...
function toUiStatus(status: ProcessStatusApi): ProcessStatusUi {
  switch (status) {
    case "ACTIVE":
      return "Aktif"
    case "PASSIVE":
...
export default function AboutUsProcessListPage() {
  ...
  async function fetchItems() {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || "Process kayıtları alınamadı.")
      }
      ...
      const mapped: ProcessItem[] = rawList.map((item: any, index: number) => ({
        id: Number(item.id ?? index + 1),
        no: String(item.no ?? `${index + 1}`.padStart(2, "0")),
        title: item.title ?? "",
        description: item.description ?? "",
        status: toUiStatus(item.status as ProcessStatusApi),
      ...
      console.error("Process list fetch error:", error)
      setError("Process kayıtları yüklenirken hata oluştu.")
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  async function handleDelete(id: number) {
    const ok = window.confirm("Bu process kaydını silmek istediğine emin misin?")
    if (!ok) return

    try {
      setDeletingId(id)

      const res = await fetch(`${PROCESS_ENDPOINT}/${id}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
        },
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-3xl xl:text-4xl">
              Process Alanını Yönet
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Süreç adımlarını, sıralamayı ve görünürlüğü buradan yönetebilirsin.
            </p>
          </div>
        ...
        {[
          {
            ...
            note: "Process alanı kayıtları",
            icon: Workflow,
          },
          {
            title: "Aktif Kayıt",
            value: activeItems.toString(),
            note: "Yayında olan adımlar",
            icon: CheckCircle2,
          },
          {
            title: "Pasif Kayıt",
        ...
        <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Arama yap, filtrele ve süreç adımlarını yönet
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row">
            <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
              ...
              <input
                ...
                placeholder="Adım veya açıklama ara..."
          ...
          <>
              ...
              {paginatedItems.map((item) => (
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <Link
                      href={`/about-us/process/${item.id}/edit`}
                      className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"
                    >
            ...
            <div className="hidden overflow-hidden rounded-[22px] border border-slate-200 xl:block">
              <div className="grid grid-cols-12 bg-slate-50 px-5 py-4 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                ...
                <div className="col-span-5">Süreç Adımı</div>
                ...
                {paginatedItems.map((item) => (
                  ...
                  >
                    <div className="col-span-1 flex justify-center">
                      <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-indigo-100 bg-indigo-50 text-sm font-bold text-indigo-600">
                        {item.no}
                      </span>
                    </div>
