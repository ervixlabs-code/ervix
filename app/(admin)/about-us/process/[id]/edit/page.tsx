...
export default function EditAboutProcessPage() {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        ...
        console.error("Edit about process fetch error:", error)
        setNotFound(true)
      } finally {
        setIsLoaded(true)
      }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text()
        throw new Error(text || "Process kaydı güncellenemedi.")
      }

      router.push("/about-us/process")
    } catch (error) {
      console.error("Edit about process error:", error)
      alert("Process kaydı güncellenirken hata oluştu.")
    } finally {
  ...
  if (notFound) {
    return (
      <div className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <h1 className="text-2xl font-bold text-slate-900">Process kaydı bulunamadı</h1>
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-3xl xl:text-4xl">
              Process Kaydını Düzenle
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Süreç adımının numara, başlık, açıklama ve sıralama alanlarını güncelleyebilirsin.
            </p>
          </div>
        ...
        <div className="2xl:col-span-7 space-y-6">
            ...
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Süreç adımının numara, başlık ve açıklama alanlarını güncelle
                </p>
              </div>
            </div>
          ...
          <section className="flex flex-col gap-3 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Form bilgilerini kontrol edip süreç adımını güncelleyebilirsin.
              </p>
            </div>
