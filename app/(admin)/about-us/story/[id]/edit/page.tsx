...
function mapApiStatusToUi(value: any): StoryStatus {
  if (value === "ACTIVE" || value === "Aktif" || value === "active") return "Aktif"
  return "Pasif"
}

function mapUiStatusToApi(value: StoryStatus) {
...
function pickStoryRecord(payload: any) {
...
export default function EditAboutStoryPage() {
    ...
    const fetchItem = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(errorText || "Kayıt getirilemedi.")
        }

        const text = await res.text()
        const parsed = safeJsonParse(text)
        const item = pickStoryRecord(parsed)
        ...
        setForm({
          ...
          storyParagraph1:
            ...
            item.paragraph1 ??
            item.storyParagraph ??
            item.story_paragraph_1 ??
            "",
          storyParagraph2:
            item.storyParagraph2 ??
            item.paragraph2 ??
            item.story_paragraph_2 ??
            "",
          missionEyebrow:
            item.missionEyebrow ??
            item.mission_eyebrow ??
            "Misyon",
          missionDescription:
        ...
        console.error("Edit about story fetch error:", error)
        setNotFound(true)
      } finally {
        setIsLoaded(true)
      }
  ...
  try {
    ...
    if (!res.ok) {
      const text = await res.text()
      throw new Error(text || "Story kaydı güncellenemedi.")
    }

    router.push("/about-us/story")
  } catch (error: any) {
    console.error("Edit about story submit error:", error)
    alert(error?.message || "Story kaydı güncellenirken hata oluştu.")
  } finally {
  ...
  if (notFound) {
    return (
      <div className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <h1 className="text-2xl font-bold text-slate-900">Story kaydı bulunamadı</h1>
        ...
        <div className="relative z-10 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-3xl xl:text-4xl">
              Story Kaydını Düzenle
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80 sm:text-[15px]">
              Hikâyemiz, misyon ve vizyon bloklarını buradan güncelleyebilirsin.
            </p>
          </div>
        ...
        <div className="2xl:col-span-7 space-y-6">
            ...
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Sol büyük kartta görünecek alanları güncelle
                </p>
              </div>
            </div>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:p-6">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Sağ üst kartta görünecek alanları güncelle
                </p>
              </div>
            </div>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:p-6">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Sağ alt kartta görünecek alanları güncelle
                </p>
              </div>
            </div>
          ...
          <section className="flex flex-col gap-3 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Form bilgilerini kontrol edip story kaydını güncelleyebilirsin.
              </p>
            </div>
